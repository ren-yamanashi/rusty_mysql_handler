...
User-facing command: `cargo build --release -p engine`. `src/sys_bindings.rs` is a committed bindgen output, so consumers don't need MySQL headers (or the submodule) for `cargo check` / `cargo test`. The shim staticlib is picked by env var:
...
| (no env var) | No shim linking. Sufficient for `cargo check` / `cargo test`; the resulting cdylib is not loadable into mysqld |

`src/sys_bindings.rs` is regenerated by `MYSQL_HANDLER_REGEN_BINDINGS=1 cargo build -p mysql-handler` (requires `make setup`). That env var is the only trigger for bindgen.
