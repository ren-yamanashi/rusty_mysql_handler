...
```mermaid
graph TD
  A["MySQL Server (mysqld, C++)"] -->|"dlopen + dlsym"| B
  subgraph cdylib["libengine.so / libengine.dylib (cdylib)"]
    B["plugin manifest<br/>(3 Rust pub no_mangle statics)"]
    B --> C["examples/engine: StorageEngine impl"]
    C --> D["mysql-handler rlib:<br/>StorageEngine trait + ffi + sys"]
    D --> E["ha_rusty_shim (C++ staticlib)<br/>RustHandlerBase, thr_lock helpers"]
  end
  E -. "C++ symbols<br/>(handler base class, etc.)" .-> A
```
...
User-facing command: `cargo build --release -p engine`. The mysql-handler crate's `build.rs` picks one of three modes by env var:

| Trigger | Behaviour |
| --- | --- |
| `MYSQL_HANDLER_FROM_SOURCE=1` | `cmake` builds `libha_rusty_shim.a` from `shim/` using the `mysql-server/` submodule headers |
| `MYSQL_HANDLER_ARCHIVE=<local path>` | gunzip the named archive into `OUT_DIR/prebuilt/libha_rusty_shim.a` and link it. Local filesystem only; `build.rs` never fetches from the network |
| (no env var) | Run bindgen only. Suitable for `cargo check` / `cargo test`; the resulting cdylib is not loadable into mysqld |

## Smoke testing

`make test_e2e` builds a two-stage Docker image (builder: `rust:1.95.0-slim` + apt toolchain → `libengine.so`; runtime: `mysql:8.4.9` with the plugin copied to `plugin_dir`), starts mysqld in a container, then runs `INSTALL PLUGIN` + `tests/e2e/test.sql` via `docker exec` and asserts `SELECT COUNT(*) = 3`.

Building both the plugin and mysqld inside the same Linux container side-steps the host (often macOS) / container OS/ABI mismatch — a host-built Mach-O cannot be dlopened by a Linux mysqld and vice versa.
